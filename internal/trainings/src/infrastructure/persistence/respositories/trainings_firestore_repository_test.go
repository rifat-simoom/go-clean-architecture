...
func newFirebaseRepository(t *testing.T) respositories.TrainingsFirestoreRepository {
	t.Helper()
	firestoreClient, err := firestore.NewClient(context.Background(), os.Getenv("GCP_PROJECT"))
	require.NoError(t, err)

	return respositories.NewTrainingsFirestoreRepository(firestoreClient)
}
